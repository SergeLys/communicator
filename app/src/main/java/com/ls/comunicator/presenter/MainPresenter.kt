...
import com.ls.comunicator.activity.TableContentFragment
...
class MainPresenter(private val view: MainActivity ,private val model: CardModel) {
    ...
    private fun initTabs(pages: ArrayList<String>?, adapter: CardAdapter) {
        ...
        if (pages != null && pages.size >= 1)
            fragmentAdapter.addFragment(TableContentFragment(tabs, pages), "Оглавление")
        pages?.forEachIndexed { index, s ->
            try {
                val pageName = s.split("_")[0]
                fragmentAdapter.addFragment(PageFragment(getCardAmount(view.baseContext), adapter, s), pageName)
            } catch (e: Exception) {
                e.printStackTrace()
            }
        }
